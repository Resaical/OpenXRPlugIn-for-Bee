...
inline std::string GetEnv(const std::string& variable) {
    ...
    // Use _dupenv_s to safely get the environment variable value
    if (_dupenv_s(&value, &len, variable.c_str()) == 0 && value != nullptr) {
        std::string result(value); // Copy the value into a std::string
