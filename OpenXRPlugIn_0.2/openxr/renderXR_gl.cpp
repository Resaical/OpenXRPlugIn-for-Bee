...
void RendererXR::CreateFrameBuffers()
{
    m_width = Engine.ECS().GetSystem<OpenxrPlugIn>().m_viewConfigurationViews[0].recommendedImageRectWidth;     //INTRUSION
    m_height = Engine.ECS().GetSystem<OpenxrPlugIn>().m_viewConfigurationViews[0].recommendedImageRectHeight;   //INTRUSION
...
void RendererXR::DeleteFrameBuffers()
{
...
void RendererXR::DeleteShadowMaps()
{
    glDeleteTextures(6, m_shadowMaps);
    glDeleteFramebuffers(6, m_shadowFBOs);
}

void RendererXR::RenderCurrentInstances(int instances)
{
    glBindBuffer(GL_UNIFORM_BUFFER, m_transformsUBO);
    glBufferData(GL_UNIFORM_BUFFER, sizeof(TransformsUBO), m_transformsData, GL_DYNAMIC_DRAW);
    BEE_DEBUG_ONLY(glBindBuffer(GL_UNIFORM_BUFFER, 0));
...
void bee::RendererXR::SetFog(vec4 fogColor, float forNear, float fogFar)
{
...
void RendererXR::CreateShadowMaps()
{
...
void bee::RendererXR::BeguinRenderXR() 
{ 
    Engine.ECS().GetSystem<OpenxrPlugIn>().RenderXRBeguin();        //INTRUSION
}

void RendererXR::RenderShadowMaps()
{
    BEE_PROFILE_FUNCTION();
    int i = 0;
    for (const auto& [lightEntity, lightTransform, light] : Engine.ECS().Registry.view<Transform, Light>().each())
    {
        if (light.Type == Light::Type::Directional && i < m_max_dir_lights && light.ShadowExtent > 0)
        {
...
void bee::RendererXR::DeleteUBOs()
{
...
void RendererXR::SetAlphaBlending2D(bool value) { m_useAlphaBlending = value; }
void RendererXR::SetVignette(float value) { m_vignette = glm::clamp<float>(value, 0.f, 1.f); }

void RendererXR::Render() 
{ 
   Engine.ECS().GetSystem<OpenxrPlugIn>().RenderXRBeguin();  // Contains the eye loop
}

// Common flat rendering
void bee::RendererXR::RenderFlat()    
{
...
#pragma endregion
    ...
    for (const auto& [e, camera, cameraTransform] : Engine.ECS().Registry.view<Camera, Transform>().each())
    {
        ...
        worldCameraTransform.SetFromMatrix(cameraTransform.World());
        const vec4 eyePos = vec4(worldCameraTransform.GetTranslation(), 1); 
        ...
        m_cameraData->bee_resolution = vec2(m_width, m_height);
...
} //Actual common flat rendering.

void bee::RendererXR::EndRenderXR() {}

void RendererXR::ProcessObjectForRendering(const MeshRenderer& renderer, Transform& transform, int& instances)
{
    // Check if end of batch is reached
...
void bee::RendererXR::LoadEnvironment(FileIO::Directory directory, const std::string& filename)
{
    /////////////////////////////////////// Load HDR texture /////////////////////////////////////////////
    {
...
void RendererXR::ApplyMaterial(const std::shared_ptr<Material>& material)
{
...
void RendererXR::Inspect(bee::Entity e)
{
    ImGui::PushID((void*)e);
